...
class RetryParsingSyncService : Service() {
 ...
 //   private val serviceScope = CoroutineScope(Dispatchers.IO + Job())
    ...
    private val runnableGetApiStart: java.lang.Runnable = object : java.lang.Runnable {
        ...
        override fun run() {
            dnFailedViewModel.readAllData.observe(ProcessLifecycleOwner.get(),
                Observer { files ->
                    if (files.isNotEmpty()) {
    ...
    @SuppressLint("SetTextI18n")
    private fun downloadSequentially(files: List<DnFailedApi>) {
    ...
    private fun checkWhatAreaToDownloadFromDueToErrorIssues(
        ...
        GlobalScope.launch(Dispatchers.IO){
            ...
            downloadSequentially(dnFailedViewModel.readAllData.value ?: emptyList())
